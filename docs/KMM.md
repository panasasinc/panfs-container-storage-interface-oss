...
KMM is configured via a `Module` custom resource. For PanFS, see the `Module` manifest in `deploy/k8s/csi-driver/template-csi-panfs.yaml`:
    ...
    container:
      ...
      kernelMappings:
        - literal: 4.18.0-553.el8_10.x86_64
          containerImage: <your-registry>/<repository>/panfs-dfc-kmm:4.18.0-553.el8_10.x86_64
  imageRepoSecret:
...
- **moduleName**: Name of the kernel module to load
...
- `<your-registry>/<repository>` — the container registry and repository
...
- [KMM Module Example](../deploy/k8s/csi-driver/template-csi-panfs.yaml)
